    ...
    if (!$errores) {
        ...
        if (!$usuario) {
            ...
            if ($usuario['alumno_id'] && !empty($usuario['alumno_dni'])) {
                ...
                // Si es coach, dueño o superadmin sin DNI en ficha alumnos, verificar en alumnos del mismo gimnasio o permitir validación
                $stDniCheck = $pdo->prepare("SELECT id FROM alumnos WHERE gimnasio_id = ? AND dni = ? LIMIT 1");
                $stDniCheck->execute([$usuario['gimnasio_id'], $dniClean]);
                if ($stDniCheck->fetch()) {
                    $dniValido = true;
                } else {
                    // Si el usuario no tiene DNI asociado pero coincide exactamente su identificador y correo
                    if (filter_var($identificador, FILTER_VALIDATE_EMAIL) && strtolower($identificador) === strtolower($usuario['email'])) {
                        $dniValido = true;
                    }
                }
            }
            ...
            if (!$dniValido) {
                $errores[] = 'El DNI ingresado no coincide con los datos registrados para esta cuenta.';
            } else {
  ...
  <div class="auth-card">
    ...
    <form method="post" action="recuperar_password.php">
      ...
      <div class="auth-form-group">
        ...
        <input class="inp" type="password" name="password2" required placeholder="Repetir nueva contraseña">
      </div>
    ...
    <div class="auth-footer">
      ¿Te acordaste de tu clave? <a href="login.php">Volver a Iniciar Sesión</a>
    </div>
  </div>
</div>
</body>
