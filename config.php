...
if (!function_exists('validatePasswordStrength')) {
    /**
     * Valida la fortaleza de la contraseña según la política estándar unificada del sistema:
    ...
    function validatePasswordStrength(string $password): array {
        ...
        if (!preg_match('/[^a-zA-Z0-9\s]/', $password)) {
            $errores[] = 'La contraseña debe contener al menos un símbolo especial (ej: ! @ # $ % * - _ . ? & +).';
        }
        return [
            'ok' => empty($errores),
            'errores' => $errores,
